...
var stringifyObj = function(obj) {
  if (!_.isPlainObject(obj) && !_.isArray(obj)) return obj;

  var arr = _.map(obj, function(val, key) {
    if (_.isPlainObject(val) || _.isArray(val)) return key + ':' + stringifyObj(val);
    else return key + ':' + val;
  });

  return _(arr).sort().value().toString();
};
